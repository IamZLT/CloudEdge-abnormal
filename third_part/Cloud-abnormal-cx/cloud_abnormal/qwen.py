...
class FrozenQwenInspector:
    def __init__(
        ...
        # Cache fingerprint must include everything that changes the inference
        # result: model identity (2B vs 9B), pixel budget, and generation config.
        # Otherwise a 2B run's cache is silently reused by a 9B run.
        self._model_path = str(path)
        self._min_pixels = int(min_pixels) if min_pixels is not None else None
    ...
    def _cache_path(self, category: str, target: Path, references: list[Path]) -> Path:
        stat = target.stat()
        signature = (
            f"{self._model_path}:{self._min_pixels}:{self._max_pixels}:{self.max_new_tokens}:"
            f"{target.resolve()}:{stat.st_size}:{stat.st_mtime_ns}:"
