#!/usr/bin/env python3
"""ROI-conditioned VLM bench on the NEW mvtec_anomaly_llm train/test split.

Pipeline (the "contrast → zoom → LLM" approach):
  1. edge patch-gallery AD builds a few-shot gallery from the TEST set normal
     images (test/{cat}/good), with leave-one-out for good queries.
  2. anomaly map → top-k ROI crops (zoom into suspected region).
  3. Qwen3.5-0.8B VLM decides OK/NG on full image vs ROI crops.

Reads the manifests written by build_mvtec_llm_split.py:
  - test.jsonl  → query set (good + defect)
  - gallery     → test/{cat}/good (label == 0 in test.jsonl)

Does NOT modify existing scripts/outputs. Writes to results_dir from config
(default: outputs/qwen35_roi_newsplit/).

Env: conda activate clip
Example:
  CUDA_VISIBLE_DEVICES=4 python scripts/bench_roi_vlm_newsplit.py \
    --config configs/qwen35_roi_newsplit.yaml --max-images 200
"""
...
def phase_extract_rois(
    ...
    """Gallery = test/good (few-shot from test, leave-one-out); score + ROI each query."""
    ...
    for cat, cat_rows in sorted(by_cat.items()):
        good_rows = [r for r in cat_rows if int(r["label"]) == 0]
        good_paths = [Path(r["image"]) for r in good_rows]
        print(
            f"[roi] gallery {cat}: {len(good_paths)} test/good (few-shot) "
            f"→ {len(cat_rows)} queries (leave-one-out for good)"
        )
        # pre-encode support tokens once
        good_pts = [ad.encode_patches(Image.open(p).convert("RGB")) for p in good_paths]

        for i, row in enumerate(cat_rows):
            img_path = Path(row["image"])
            pil = Image.open(img_path).convert("RGB")
            if int(row["label"]) == 0:
                # leave-one-out: exclude self from support
                pts = [
                    pt
                    for gp, pt in zip(good_paths, good_pts)
            ...
            entry = {
                ...
                "category": cat,
                "label": int(row["label"]),
                "defect_type": str(row.get("defect_type") or "none"),
            ...
            print(
                ...
                f"gt={'NG' if row['label'] else 'OK'} score={score:.3f} "
                f"gal={len(pts)} n_roi={len(rois)}"
            )
...
def edge_metrics(caches: list[dict]) -> dict:
    y_true = [int(e["label"]) for e in caches]
    scores = [float(e["edge_score"]) for e in caches]
    yt = np.asarray(y_true, dtype=int)
    sc = np.asarray(scores, dtype=float)
    auroc = float(roc_auc_score(yt, sc)) if len(np.unique(yt)) > 1 else float("nan")
    f1, prec, rec, thr = best_f1_threshold(yt, sc)
    ...
    return {
        ...
        "recall": rec,
        "threshold": float(thr),
        "pred_ok": int(sum(1 for p in pred if p == 0)),
        "pred_ng": int(sum(1 for p in pred if p == 1)),
    }
...
def run_vlm_mode(client, caches, mode, prompt) -> dict:
    ...
    for i, e in enumerate(caches):
        ...
        if img is None:
            decision, conf = "OK", 0.4
            defect_type, reason, latency, raw = "none", "no salient ROI", 0.0, ""
        else:
...
def main():
    ...
    parser.add_argument("--config", default=str(ROOT / "configs/qwen35_roi_newsplit.yaml"))
    parser.add_argument("--max-images", type=int, default=None, help="cap balanced query set (None=all)")
    ...
    out_dir = Path(cfg.get("results_dir") or "outputs/qwen35_roi_newsplit")
    if not out_dir.is_absolute():
        out_dir = ROOT / out_dir
    out_dir.mkdir(parents=True, exist_ok=True)

    test_jsonl = Path(cfg.get("test_jsonl") or "datasets/mvtec_anomaly_llm/test.jsonl")
    if not test_jsonl.is_absolute():
        test_jsonl = ROOT / test_jsonl
    rows = subsample_balanced(load_jsonl(test_jsonl), args.max_images)
    ...
    out_json = out_dir / "bench_roi_newsplit.json"
    out_md = out_dir / "bench_roi_newsplit.md"
    out_json.write_text(json.dumps(report, indent=2, ensure_ascii=False), encoding="utf-8")

    lines = [
        "# ROI-conditioned Qwen3.5-0.8B bench — NEW mvtec_anomaly_llm split (test/good few-shot)",
        "",
        f"- Test manifest: `{test_jsonl}`",
        f"- N: {len(caches)}",
        f"- Adapter: `{adapter or 'none (zero-shot)'}`",
        f"- Edge gallery: **test/good** (few-shot from test split, leave-one-out; no fixed 16-shot)",
        "",
        "## Edge-only (contrast detector, threshold = best-F1 on query)",
        "",
        f"- AUROC: **{edge_m['auroc']:.4f}**",
        f"- F1: **{edge_m['f1']:.4f}**  P={edge_m['precision']:.4f} R={edge_m['recall']:.4f} "
        f"thr={edge_m['threshold']:.4f} pred {edge_m['pred_ok']}/{edge_m['pred_ng']}",
        "",
        "## VLM (contrast → zoom → LLM)",
        "",
    ...
    lines += [
        "## Notes",
        "",
        "- Gallery is test/{cat}/good (few-shot support from the test split), NOT train/good 16-shot.",
        "- leave-one-out: a good query is removed from its own gallery before scoring.",
        "- `full`: whole image; `roi_top1`/`roi_collage`: crops from edge contrast anomaly map.",
        "",
    ]
