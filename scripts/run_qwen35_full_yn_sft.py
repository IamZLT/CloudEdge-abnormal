#!/usr/bin/env python3
"""Full-image + Yes/No prompt LoRA pipeline: build → balance → train → eval.

The corpus is trivial (no edge model / ROI): each train image keeps its FULL
path, prompt is the fixed Yes/No question, response is "Yes" (NG) or "No" (OK).
OK/NG are balanced 1:1 per category. Then LoRA-train and evaluate on the test
split with the SAME Yes/No prompt.

Env: conda activate clip
  CUDA_VISIBLE_DEVICES=6 python scripts/run_qwen35_full_yn_sft.py \
    --config configs/qwen35_full_yn_sft.yaml --device cuda:0
"""
...
def build_corpus(cfg: dict, out_dir: Path) -> Path:
    ...
    out_path = out_dir / "full_yn_train.jsonl"
...
def stage_eval(cfg: dict, device: str, out_dir: Path, adapter: Path):
    ...
    cmd = [
        sys.executable,
        "-u",
        str(ROOT / "scripts" / "bench_zero_shot_yn.py"),
        "--test-jsonl",
        str(test_path),
        "--out",
        str(out_json),
...
def main():
    ...
    parser.add_argument("--config", default=str(ROOT / "configs/qwen35_full_yn_sft.yaml"))
    ...
    out_dir = Path(cfg.get("results_dir") or "outputs/qwen35_full_yn_sft")
    if not out_dir.is_absolute():
        out_dir = ROOT / out_dir
    out_dir.mkdir(parents=True, exist_ok=True)

    corpus = out_dir / "full_yn_train.jsonl"
    ...
    if args.stage in ("train", "all"):
        if not (adapter / "adapter_model.safetensors").exists():
            ...
            bal_jsonl = out_dir / "full_yn_train_balanced.jsonl"
            val_jsonl = out_dir / "full_yn_val_balanced.jsonl"
